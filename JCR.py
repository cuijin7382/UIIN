    ...
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, depth=3):
        ...
        for i in range(depth - 1):
            ...
            self.skip_conv.append(
                nn.Conv2d(n_feat + scale_unetfeats * (depth - i - 1), n_feat + scale_unetfeats * (depth - i - 2),
...
class DownSample(nn.Cell):
    def __init__(self, in_channels, s_factor):
        ...
        self.down = nn.SequentialCell(nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=False),
                                      nn.Conv2d(in_channels, in_channels + s_factor, kernel_size=1, stride=1, padding=0,
                                                has_bias=False))
...
class JCRNet(nn.Cell):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3,
                 reduction=4, bias=False, depth=1):
